...
Single-page React app — a visual finite-state machine (FSM) editor built on **React Flow** (`@xyflow/react`) with **Zustand** state management and **ELK.js** available for layout.

### Data model (`lib/types.ts`)
- `StateNode` — React Flow node with `StateNodeData` (`name`, `onEnter`, `onExit`, `isInitial`)
- `TransitionEdge` — React Flow edge with `TransitionEdgeData` (`trigger`, `guard`, `action`, `routedPoints`)
...
### Edge routing (`lib/routing.ts`)
Custom A* pathfinder on a 10px grid. `routeAllEdges()` avoids obstacles (node bounding boxes + padding) with a turn penalty. `useEdgeRouting()` hook debounces re-routing (80 ms) on node/edge changes and writes results back via `setRoutedPoints`. Self-loops use a fixed offset path.
